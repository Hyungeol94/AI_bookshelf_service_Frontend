...
export default (props) => {
  ...
  const handleBookshelf = async () => {
    ...
    const handleBookshelf = () => {
      if (bookshelf) {
        // eslint-disable-next-line no-restricted-globals
        var result_bookshelf = confirm("내 서제에서 삭제하시겠습니까?");
        if (result_bookshelf === true) {
          setBookshelf(false);
          // await api.deleteBookshelf(request);
        }
      } else {
        // eslint-disable-next-line no-restricted-globals
        var result_bookshelf = confirm("내 서재에 추가하시겠습니까?");
        if (result_bookshelf === true) {
          setBookshelf(true);
          // await api.addBookshelf(request);
        }
        ...
        >
          ...
          >
            ...
            >
              ...
              <h4
                style={{ marginBottom: 0, fontSize: "14px", textAlign: "end" }}
              >
